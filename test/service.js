...
function updateLimiter(identifier, tokens) {
  return pool.connect()
    .then(client => 
      client.query(`UPDATE limiters SET tokens=${tokens}, last_update=NOW() WHERE identifier='${identifier}'`)
        .finally(() => 
          client.release()
        )
    )
...
function updateAuthorization(sessionToken, expiration) {
  return pool.connect()
    .then(client => 
      client.query(`UPDATE authorizations SET expires_at=TO_TIMESTAMP(${expiration}) WHERE session_token='${sessionToken}'`)
        .finally(() => 
          client.release()
        )
    )
...
function deleteAuthorizations() {
  return pool.connect()
    .then(client => 
      client.query(`DELETE FROM authorizations`)
        .finally(() => 
          client.release()
        )
    )
...
describe('service', () => {
  ...
  describe('authorization route', () => {
    ...
    describe('/auth/authorize/:authorizationCode', () => {
      ...
      it('should return 400 with bad authorization code', () => { 
      ...
      it('should return 503 with no day tokens', () => { 

        return updateLimiter('strava.day', 0)
          .then(() => 
            chai.request(app)
              .post('/auth/authorize/1376438746283746516514134')
              .then(response => {
      ...
      it('should return 503 with no minute tokens', () => { 

        return updateLimiter('strava.minute', 0)
          .then(() => 
            chai.request(app)
              .post('/auth/authorize/1376438746283746516514134')
              .then(response => {
  ...
  describe('protected routes', () => {
    ...
    describe('/auth/deauthorize', () => {
      ...
      it('should return 400 with missing authorization header', () => { 
      ...
      it('should return 400 with bad authorization header', () => {
      ...
      it('should return 401 with invalid session token', () => {
      ...
      it('should return 401 with expired session token', () => {

        return updateAuthorization(sessionToken, moment().subtract(1, 'second').unix())
          .then(() =>
            chai.request(app)
            .post('/auth/deauthorize')
      ...
      it('should return 200 and revoke authorization with valid authorization header', () => { 
        ...
        return chai.request(app)
          ...
          .then(response => {
            chai.expect(response).to.have.status(200)
            chai.expect(response).to.be.json
            chai.expect(response.body).to.have.keys(['status'])
    ...
    describe('/api/v1/activities/:before/:after', () => {
      ...
      it('should return 400 wth bad before timestamp', () => { 
      ...
      it('should return 400 with bad after timestamp', () => { 
