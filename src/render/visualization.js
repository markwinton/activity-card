import { Box3, SphereGeometry, MeshBasicMaterial, Mesh, AmbientLight } from 'three'
import { clamp } from './math.js'
import { mean, standardDeviation } from './statistics.js'
import { goldenAngle, vogelSpiral, easeOut } from './graphics.js'
...
export default class {
  ...
  constructor(activities, scene) {
    ...
    activities.forEach((activity, i) => {
      ...
      const radius = RADIUS.min + (distance + distanceSigma) / (distanceSigma * 2) * (RADIUS.max - RADIUS.min) 
      const opacity = OPACITY.min + (speed + speedSigma) / (speedSigma * 2) * (OPACITY.max - OPACITY.min)
      ...
      const sphere = new Mesh(
        ...
        new MeshBasicMaterial({ transparent: true, opacity: 0, color: color })
      )
      sphere.position.set(position.x, position.y, 0)
      sphere.userData = { opacity: opacity }
    ...
    this.animation = setInterval(() => {
      ...
      this.spheres.forEach((sphere, i) => {
        ...
        sphere.material.opacity = sphere.userData.opacity * p
        sphere.scale.set(
          0.1 + p * 0.9, 
          0.1 + p * 0.9, 
          0.1 + p * 0.9
