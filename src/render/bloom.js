import {
  PlaneGeometry, Mesh, OrthographicCamera, Scene, WebGLRenderTarget,
} from 'three';
...
export default class {
  constructor() {
    ...
    const blur = this.mipmap.slice(1).map(level => level.y.texture);

    this.blurShader = new BlurShader();

    this.compositeShader = new CompositeShader();
    this.compositeShader.uniforms.textures.value = blur;
  }
  ...
  render(renderer, target) {
    ...
    for (let i = 1; i < this.mipmap.length; i += 1) {
